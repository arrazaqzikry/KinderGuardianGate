...
async function simulateScan() {
    ...
    if (!parentName) {
        showNotification('Please enter a parent name');
        return;
    }
    ...
    try {
        const response = await fetch('/api/guardians/verify', {
            ...
            body: `parentName=${encodeURIComponent(parentName)}`
        });
        ...
        if (result.status === 'success') {
            ...
            addPickupToTable(parentName, '-', 'UNAUTHORIZED');
            showNotification(`Unauthorized pickup attempt for ${parentName}`);
        }
